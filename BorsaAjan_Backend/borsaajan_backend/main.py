...
@app.get("/test/add/{sembol}")
def test_verisi_ekle(sembol: str, fiyat: float, karar: str = "AL"):
    """
    Test için önceki analiz verisi ekle.
    Örnek: http://127.0.0.1:8000/test/add/NVDA?fiyat=150&karar=AL
    """
    add_test_data(sembol.upper(), fiyat, karar)
    return {
        "mesaj": f"Test verisi eklendi: {sembol.upper()} - ${fiyat} - {karar}",
        "not": "Şimdi /analiz/{sembol} endpoint'ini çağırarak self-learning'i test edebilirsiniz"
    }

@app.get("/test/chart/{sembol}")
def test_chart(sembol: str):
    """
    Chart fonksiyonunu test et.
    Örnek: http://127.0.0.1:8000/test/chart/NVDA
    """
    try:
        chart_base64 = create_chart(sembol.upper())
        if chart_base64:
            return {
                "durum": "Başarılı",
                "sembol": sembol.upper(),
                "base64_uzunluk": len(chart_base64),
                "base64_ilk_100_karakter": chart_base64[:100] if len(chart_base64) > 100 else chart_base64,
                "mesaj": "Chart başarıyla oluşturuldu!"
            }
        else:
            return {
                "durum": "Hata",
                "sembol": sembol.upper(),
                "mesaj": "Chart oluşturulamadı (None döndü). Backend loglarını kontrol edin."
            }
    except Exception as e:
        return {
            "durum": "Hata",
